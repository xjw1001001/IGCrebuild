        ...
        with open(output_fasta, 'w+') as g:
            for line in f.readlines():
                if line[0] == '>':
                    ...
                    assert(len(line) %3 == 1)
                    g.write(Seq.translate(line[:-1], to_stop = False) + '\n')
...
if __name__ == '__main__':
    path = '/Users/xji3/Genconv/NewClusterPackRun/ZebraFish/'

    pair = ['TLR5a', 'TLR5b']
    #mkdir_cmd = ['mkdir', '_'.join(pair)]
    MAFFT_cmd = ['/usr/local/bin/mafft', '--auto',
                 path + '_'.join(pair) + '_AA.fa', '>',
                 path + '_'.join(pair) + '_AA_MAFFT.fa']
    #cp_cmd = ['cp',
    ...
    translateDNAtoAA('./' + '_'.join(pair) + '.fa',
                     './' + '_'.join(pair) + '_AA.fa')
    os.system(' '.join(MAFFT_cmd))
    format_fasta(path + '_'.join(pair) + '_AA_MAFFT.fa',
                 path + '_'.join(pair) + '_AA_MAFFT_formated.fa')
    translateAAAlignmentDNAAlignment('./' + '_'.join(pair) + '_AA_MAFFT_formated.fa',
                                     './' + '_'.join(pair) + '.fa',
                                     './' + '_'.join(pair) + '_MAFFT.fa')

    GapRemovedFasta(processAlignment('./' + '_'.join(pair) + '_MAFFT.fa'),
                    './' + '_'.join(pair) + '_input.fasta')
