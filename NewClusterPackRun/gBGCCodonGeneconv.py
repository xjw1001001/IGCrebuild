...
class gBGCCodonGeneconv(ReCodonGeneconv):
        ...
        for i, pair in enumerate(product(self.codon_nonstop, repeat = 2)):
            ...
            if ca != cb:
                ...
                if isNonsynonymous(cb, ca, self.codon_table):
                    ...
                    Tgeneconv = self.tau
                rate_geneconv.append(Qb + Tgeneconv * self.get_GC_fitness(cb, ca))
                rate_basic.append(0.0)
                ...
                col.append((sb, sb))
                Qb = Qbasic[sa, sb]
                rate_geneconv.append(Qb + Tgeneconv * self.get_GC_fitness(ca, cb))
                rate_basic.append(0.0)
        ...
        for i, pair_from in enumerate(product('ACGT', repeat = 2)):
            ...
            for j, pair_to in enumerate(product('ACGT', repeat = 2)):
                ...
                if na ==nc and nb != nd:
                    ...
                    if na == nd:
                        GeneconvRate = Qb + self.tau * self.get_GC_fitness(nb, na)
                    else:
                        GeneconvRate = Qb
                if nb == nd and na != nc:
                    Qb = Qbasic['ACGT'.index(na), 'ACGT'.index(nc)]
                    if nb == nc:
                        GeneconvRate = Qb + self.tau * self.get_GC_fitness(na, nb)
                    else:
                        GeneconvRate = Qb
    ...
    def save_to_file(self, file_name = None, path = './'):
        if file_name == None:
            file_name = 'gBGC_' + self.Model + '_' + '_'.join(self.paralog)
            if self.clock:
                file_name += '_clock.p'
            else:
        ...
        if self.Model == 'MG94':
            ...
            for i, pair in enumerate(product(self.codon_nonstop, repeat = 2)):
                ...
                if isNonsynonymous(cb, ca, self.codon_table):
                    ...
                    Tgeneconv = self.tau
                Tgeneconv_ba = Tgeneconv * self.get_GC_fitness(cb, ca)
                proportions.append(Tgeneconv_ba / (Qb + Tgeneconv_ba) if (Qb + Tgeneconv_ba) >0 else 0.0)

                # (ca, cb) to (cb, cb)
                row_states.append((sa, sb))
                column_states.append((sb, sb))
                Qb = Qbasic[sa, sb]
                Tgeneconv_ab = Tgeneconv * self.get_GC_fitness(ca, cb)
                proportions.append(Tgeneconv_ab / (Qb + Tgeneconv_ab) if (Qb + Tgeneconv_ab) >0 else 0.0)
            ...
            for i, pair in enumerate(product('ACGT', repeat = 2)):
                ...
                Tgeneconv_ba = self.tau * self.get_GC_fitness(nb, na)
                proportions.append(Tgeneconv_ba / (GeneconvRate - self.tau + Tgeneconv_ba) if GeneconvRate > 0 else 0.0)
                ...
                Tgeneconv_ab = self.tau * self.get_GC_fitness(na, nb)
                proportions.append(Tgeneconv_ab / (GeneconvRate - self.tau + Tgeneconv_ab) if GeneconvRate > 0 else 0.0)
        ...
        if self.Model == 'MG94':
            ...
            for i, pair in enumerate(product(self.codon_nonstop, repeat = 2)):
                ...
                if isNonsynonymous(cb, ca, self.codon_table):
                    ...
                    Tgeneconv = self.tau
                Tgeneconv_ba = Tgeneconv * self.get_GC_fitness(cb, ca)
                proportions12.append(Tgeneconv_ba / (Qb + Tgeneconv_ba) if (Qb + Tgeneconv_ba) >0 else 0.0)
                ...
                Qb = Qbasic[sa, sb]
                Tgeneconv_ab = Tgeneconv * self.get_GC_fitness(ca, cb)
                proportions21.append(Tgeneconv_ab / (Qb + Tgeneconv_ab) if (Qb + Tgeneconv_ab) >0 else 0.0)
            ...
            for i, pair in enumerate(product('ACGT', repeat = 2)):
                ...
                GeneconvRate = get_HKYGeneconvRate(pair, na + na, Qbasic, self.tau)
                Tgeneconv_ba = self.tau * self.get_GC_fitness(nb, na)
                proportions12.append(Tgeneconv_ba / (GeneconvRate - self.tau + Tgeneconv_ba) if GeneconvRate > 0 else 0.0)
                ...
                GeneconvRate = get_HKYGeneconvRate(pair, nb + nb, Qbasic, self.tau)
                Tgeneconv_ab = self.tau * self.get_GC_fitness(na, nb)
                proportions21.append(Tgeneconv_ab / (GeneconvRate - self.tau + Tgeneconv_ab) if GeneconvRate > 0 else 0.0)
    ...
    def get_summary(self, output_label = False):
        ...
        if self.Model == 'HKY': # HKY model doesn't have omega parameter
            out.extend([self.kappa, self.tau, self.gamma])
            label = ['length', 'll','pi_a', 'pi_c', 'pi_g', 'pi_t', 'kappa', 'tau', 'gamma']
        elif self.Model == 'MG94':
            out.extend([self.kappa, self.omega, self.tau, self.gamma])
...
def main(args):
    ...
    test = gBGCCodonGeneconv( newicktree, alignment_file, paralog, Model = 'MG94', Force = Force, clock = False)
