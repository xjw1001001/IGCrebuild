...
def get_likelihood_nonclock(x, test, gBGC = False):
    ...
    if gBGC:
        x[3:-1] = np.log(abs(x[3:-1]))
    else:
        x[3:] = np.log(abs(x[3:]))    # no transform of other elements
...
def get_likelihood_nonclock_BF(x, test, gBGC = False):
    ...
    num_rates = len(test.x_rates)
    if gBGC:
        x[3:-(1 + num_rates)] = np.log(abs(x[3:-(1 + num_rates)]))
    else:
        x[3: -num_rates] = np.log(abs(x[3: -num_rates]))    # no transform of other elements
