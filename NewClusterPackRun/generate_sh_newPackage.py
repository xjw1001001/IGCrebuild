if __name__ == '__main__':
    pairs = []
    with open('./NewPairsAlignment/pairs_list.txt', 'r') as f:
        for line in f.readlines():
            pairs.append(line.replace('\n','').split('_'))
    pairs.remove(['YLR028C', 'YMR120C'])
    pairs.remove(['YJR031C', 'YEL022W'])
    pairs.remove(['YHL003C', 'YKL008C'])
    pairs.remove(['YAL038W', 'YOR347C'])
    Force_str = '{5:0.0}'
    ...
    with open('./CdNPack.sh', 'w+') as f:
        ...
        for pair in pairs:
            ...
            with open('./NewRun/' + '_'.join(pair) + 'Cd.sh', 'w+') as g:
                g.write('#!/bin/bash' + '\n')
                g.write('python Rewrite_CodonGeneconv.py --paralog1 ' + pair[0] + ' --paralog2 ' + pair[1] + save_txt + '\n')
    ...
    with open('./CdNPack_ForceTau.sh', 'w+') as f:
        ...
        for pair in pairs:
            ...
            with open('./NewRun/' + '_'.join(pair) + 'Cd_ForceTau.sh', 'w+') as g:
                g.write('#!/bin/bash' + '\n')
                g.write('python Rewrite_CodonGeneconv.py --paralog1 ' + pair[0] + ' --paralog2 ' + pair[1] + ' --Force '+ Force_str + save_txt + '\n')
