...
##        seqfile = path + '_'.join(pair) + '/' + '_'.join(pair) + '_input.fasta'
...
##                f.write(new_line)
##
##        outfile_codeml = path + '_'.join(pair) + '/' + '_'.join(pair) + '_codeml'
##        codeml_ctlfile = path + '_'.join(pair) + '/' + '_'.join(pair) + '_codeml_control.ctl'
##        with open(codeml_ctlfile, 'w+') as f:
##            f.writelines(['seqfile = ' + seqfile + '\n', 'treefile = ' + treefile + '\n', 'outfile = ' + outfile_codeml + '\n'])
##            f.writelines(all_codeml_ctl_lines)
##
##        codeml_cmd = [codeml, '_'.join(pair) + '_codeml_control.ctl']
##        os.chdir(path + '_'.join(pair) + '/')
##        #os.system(' '.join(codeml_cmd))
##        subprocess.check_output(codeml_cmd)
##
##        outfile_baseml = path + '_'.join(pair) + '/' + '_'.join(pair) + '_baseml'
##        baseml_ctlfile = path + '_'.join(pair) + '/' + '_'.join(pair) + '_baseml_control.ctl'
##        with open(baseml_ctlfile, 'w+') as f:
##            f.writelines(['seqfile = ' + seqfile + '\n', 'treefile = ' + treefile + '\n', 'outfile = ' + outfile_baseml + '\n'])
##            f.writelines(all_baseml_ctl_lines)
##
##        baseml_cmd = [baseml, '_'.join(pair) + '_baseml_control.ctl']
