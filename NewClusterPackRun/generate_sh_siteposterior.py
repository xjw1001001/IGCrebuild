...
##                    g.write('python get_site_posterior.py --paralog1 ' + pair[0] + ' --paralog2 ' + pair[1] + ctl_line.replace('HKY', 'MG94') + '\n')
    ...
    sh_line = 'sbatch -o Pdiff-%j.out --mail-type=FAIL --mail-user=xji3@ncsu.edu ./NewRun/'
    ctl_lines = [' --model MG94 --no-clock --no-force --no-dir --no-gBGC'] 
    with open('./plotPdiff.sh', 'w+') as f:
        f.write('#!/bin/bash' + '\n')
        for pair in pairs:
            #save_txt = ' > Hessian_' + '_'.join(pair) + '_PrintScreen.txt'
            f.write(sh_line + '_'.join(pair) + '_Pdiff.sh' + '\n')
            with open('./NewRun/' + '_'.join(pair) + '_Pdiff.sh', 'w+') as g:
                g.write('#!/bin/bash' + '\n')
                for ctl_line in ctl_lines:
