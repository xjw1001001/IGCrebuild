...
def get_geneconv_process_definition(
    ...
    # Use this to compute the expected rate.
    nstates = 61
    row_indices = []
    column_indices = []
    ...
    column_states = []
    transition_rates = []
    row_idx_to_info = {i : [] for i in range(nstates)}
    for (i, j), r in zip(state_pairs, rates):
        row_idx_to_info[i].append([j, r])
    for ia, ib in itertools.product(range(61), repeat=2):
        for j, r in row_idx_to_info[ia]:
            row_states.append([ia, ib])
            column_states.append([j, ib])
            transition_rates.append(r)
        for j, r in row_idx_to_info[ib]:
            row_states.append([ia, ib])
            column_states.append([ia, j])
            transition_rates.append(r)
...
def main():
    ...
    with open('collapsed.tree.newick') as fin:
        lines = fin.readlines()
